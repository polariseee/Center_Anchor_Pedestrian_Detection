...
class KpHead(nn.Module):
    def __init__(self,
        ...
        self.lateral_connect = lateral_connect
    ...
    def forward(self, inputs):
        ...
        if self.lateral_connect:
            la_features = [
                la_conv(inputs[i])
                for i, la_conv in enumerate(self.la_convs)
            ]

            for i in range(self.fusion_level + 1, self.end_level + 1):
                deconv_outs.append(self.deconvs[i](la_features[i]))
        else:
            for i in range(self.fusion_level + 1, self.end_level + 1):
                deconv_outs.append(self.deconvs[i-1](inputs[i]))

        norm_outs = []
        for i in range(len(deconv_outs)):
            norm_outs.append(self.l2_norms[i](deconv_outs[i]))
